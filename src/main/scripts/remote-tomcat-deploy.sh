#!/bin/bash
# Script will copy Reesmo WAR and deploy it to tomcat

function usage() {
    echo "Deploys Reesmo WAR to given server"
    echo ""
    echo "Options:"
    echo "  -s  server where WAR will be deployed (i.e. myserver.mydomain.com)"
    echo "  -w  path to WAR to be deployed (i.e. target/reesmo-0.0.1-SNAPSHOT.war)"
...
rsync -e ssh ${WAR_LOCATION} ${TARGET_USER}@${TARGET_SERVER}:/home/${TARGET_USER}/reesmo.war
if [ $? -ne 0 ]; then
	echo "ERROR Unable to copy WAR ${WAR_LOCATION} to server ${TARGET_SERVER}.";
	exit 1;
else
	echo "================================================================================"
	echo "                         DEPLOYING REESMO                                      "
	echo "================================================================================"
ssh ${TARGET_USER}@${TARGET_SERVER} << "ENDSSH"
#!/bin/bash
NEW_BUILD="/home/$USER/reesmo.war"
...
# must be writable
TOMCAT_APP_WAR="$TOMCAT_WEBAPPS_DIR/reesmo.war"
...
then
  ...
  mv -b ${TOMCAT_APP_WAR} "$BACKUP_DIR/reesmo"_$DATE.war
fi
